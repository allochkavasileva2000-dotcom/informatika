...
int main() {
    // Создание окна
    sf::RenderWindow
    window(sf::VideoMode(800, 600), "Graph Application");
    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
            if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                float y1 = -mathX * mathX + 2*mathX + 10; //red
                float y2 = mathX * mathX + 2*mathX - 10; //blue
                
                float x1p =  - sqrt(10.0f); //точки пересечения по Ох
                float x2p =   sqrt(10.0f);
                std::string result;

                if (mathY < y2 and mathY > y1 ) {
                    result = "area 4";
                }
                ...
                coordinatesText.setString("Coordinates: (" + std::to_string(mathX) + std::to_string(mathY) +") "+  result);
            }
        }
    }
        ...
        // 5 _  Отрисовка графика y1 = x*x + 2*x - 10  (Замените на ваш график)
        drawGraph(window, [](float x) { return  x*x + 2*x - 10 / 0.5f; }, -10, 10, 30, 2, sf::Color::Blue);

        // 5 _   Отрисовка графика y2 = -x*x +2*x +10 (Замените на ваш график)
        drawGraph(window, [](float x) { return -x*x +2*x +10/ 0.5f; }, -10, 10, 20, 2, sf::Color::Red);
