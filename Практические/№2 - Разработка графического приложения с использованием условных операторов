    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
            if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (mathY > y1 and mathY < y2 and mathX < 0) {
                    result = "area 1";
                }
        ...
        drawGraph(window, [](float x) { return  x*x + 2*x - 10; }, -10, 10, 30, 30, sf::Color::Blue);
        


        // 5 _   Отрисовка графика y2 = -x*x +2*x +10 (Замените на ваш график)
        drawGraph(window, [](float x) { return -x*x + 2*x + 10; }, -10, 10, 30, 30, sf::Color::Red);
